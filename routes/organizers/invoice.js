...
router.get('/list', helper.authenticateToken, async (req, res) => {
    ...
    if (req.token.organizerid && mongoose.Types.ObjectId.isValid(req.token.organizerid)) {
        ...
        if (organizerdata && organizerdata.status == true && organizerdata.mobileverified == true) {
            ...
            let eventBookingData = await primary.model(constants.MODELS.eventbookings, eventbookingModel).find({ eventId: { $in: allEventsId } }).sort({ _id: -1 }).select("-__v -start_timestamp -end_timestamp -userid -createdBy -updatedBy").lean();
